...
void leaf_can_filter_test_475U(struct leaf_can_filter *self, struct bite *bi)
{
	...
	printf("_voltage_V: %i\n", self->_chgc._voltage_V);
	printf("_current_A: %i\n", self->_chgc._current_A);
	assert(self->_chgc._voltage_V == 425U * 2U);
	assert(self->_chgc._current_A == -350 * 2);
	assert(self->_chgc._multiplier == 2U);
...
void leaf_can_filter_test()
{
	...
	printf("remain_cap: %f\n", chgc_get_remain_cap_kwh(&fi._chgc));
